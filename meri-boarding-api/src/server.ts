...
type ContactCmsContent = {
  sections: Record<ContactSectionKey, ContactSectionState>;
  hero: {
    subtitle: string;
    title: string;
    home: string;
...
>>>>>>> 75b23de (Implement chat session logging, admin chat analytics, and RAG/chat reliability improvements)
...
const { parseEmailList, extractEmailsFromText, sendSmtpMail, formatContactSubmission } = mailUtils;
...
async function sendSmtpMail(options: { to: string[]; subject: string; text: string; html?: string; replyTo?: string }) {
  const generalSettings = await getGeneralSettingsContent();
  const resolvedSmtpHost = String(generalSettings?.smtp?.host || '').trim();
  const resolvedSmtpPortRaw = Number(generalSettings?.smtp?.port ?? defaultSmtpPort);
  const resolvedSmtpPort = Number.isFinite(resolvedSmtpPortRaw) ? Math.round(resolvedSmtpPortRaw) : 0;
  const resolvedSmtpUser = String(generalSettings?.smtp?.user || '').trim();
  const resolvedSmtpPass = String(generalSettings?.smtp?.pass || '').trim();
  const resolvedSmtpFrom = String(generalSettings?.smtp?.from || '').trim();

  if (!resolvedSmtpHost || !resolvedSmtpPort || !resolvedSmtpFrom) {
    return { sent: false, error: 'SMTP config is missing in Admin > General Settings > Form Settings.' };
  }

  const fromEnvelope = extractEnvelopeAddress(resolvedSmtpFrom);
  ...
  if ((resolvedSmtpUser && !resolvedSmtpPass) || (!resolvedSmtpUser && resolvedSmtpPass)) {
    return { sent: false, error: 'Both SMTP_USER and SMTP_PASS must be set together.' };
  }

  try {
    const mailLogoAttachment = await (async () => {
      try {
        const response = await fetch('http://public-fe:3000/images/meri/meri-logo-black.png');
        if (!response.ok) return null;
        const contentType = String(response.headers.get('content-type') || '').trim() || 'image/png';
        const bytes = Buffer.from(await response.arrayBuffer());
        if (bytes.length < 64) return null;
        return {
          filename: 'meri-logo-black.png',
          content: bytes,
          contentType,
          cid: 'meri-logo',
        };
    ...
    const dynamicSmtpSecure = resolvedSmtpPort === 465;
    const transport = nodemailer.createTransport({
      host: resolvedSmtpHost,
      port: resolvedSmtpPort,
      secure: dynamicSmtpSecure,
      requireTLS: !dynamicSmtpSecure && smtpStartTls,
      auth: resolvedSmtpUser && resolvedSmtpPass ? { user: resolvedSmtpUser, pass: resolvedSmtpPass } : undefined,
    ...
    await transport.sendMail({
      ...
      html: String(options.html || '').trim() || undefined,
      replyTo: String(options.replyTo || '').trim() || undefined,
      attachments: mailLogoAttachment ? [mailLogoAttachment] : undefined,
    });
...
function normalizeContactContent(
  ...
  return {
    sections: contactSectionKeys.reduce(
      (acc, key, index) => {
        acc[key] = {
          enabled: Boolean(input?.sections?.[key]?.enabled ?? fallback.sections?.[key]?.enabled ?? true),
          order: Number(input?.sections?.[key]?.order ?? fallback.sections?.[key]?.order) || index + 1,
        };
        return acc;
      },
      {} as Record<ContactSectionKey, ContactSectionState>,
    ),
    hero: {
      subtitle: String(input?.hero?.subtitle ?? fallback.hero.subtitle ?? '').trim(),
      title: String(input?.hero?.title ?? fallback.hero.title ?? '').trim(),
...
function validateContactContent(input: ContactCmsContent) {
  if (!input.sections || typeof input.sections !== 'object') {
    return 'Contact sections are required';
  }
  const contactSectionKeys: ContactSectionKey[] = ['details', 'inquiry', 'bookingPartners'];
  for (const key of contactSectionKeys) {
    const section = input.sections[key];
    if (!section || typeof section !== 'object') {
      return `Contact section "${key}" is required`;
    }
    if (typeof section.enabled !== 'boolean') {
      return `Contact section "${key}" enabled must be boolean`;
    }
    if (!Number.isFinite(Number(section.order))) {
      return `Contact section "${key}" order must be numeric`;
    }
  }
  if (!input.hero.subtitle || !input.hero.title || !input.hero.home || !input.hero.crumb) {
...
async function resolveContactNotificationRecipients(locale: ContentLocale) {
  ...
  const configuredRecipients = parseEmailList(
    (generalSettings?.formDelivery?.contactNotificationEmails || []).map((item) => String(item || '')).join(','),
  );
